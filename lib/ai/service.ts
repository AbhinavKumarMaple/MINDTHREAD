...
export async function saveProcessedEntry(
  ...
  const updated = await repositories.entries.update(userId, entryId, {
    ...
    patternWhat: result.pattern?.whatIsIt ?? null,
    patternEvidence: result.pattern?.evidence ?? [],
    patternAdvice: result.pattern?.advice ?? null,
    patternAttention: result.pattern?.needsAttention ?? false,
    patternTried: false,
